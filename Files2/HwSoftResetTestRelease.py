...
class HwSoftResetTest:
    ...
    def runTest(self):
        ...
        self.logFilename = "HuaweiSoftResetTest",self.timeStamp
        ...
        for i in range(iterations):
            ...
            for chn in self.chnLst:
                ...
                try:
                    ...
                    self.it.resena(sena=1, sr=1) #hit Soft reset
                    print "Trigger Huawei Specific Soft Reset..."
                    self.mReadytime = self.readMready()#monitor mready time
                    time.sleep(1)
                    
                except:
                    raise
                self.it.connect(PORT)                #connect
                self.it.setpassword()                       #unlock x99
                it.baudrate(115200)
                time.sleep(3)
                self.it.resena(1)                                                 #turn on laser
...
hwsrt = HwSoftResetTest()

if __name__== '__main__':
    
    hwsrt.runTest()                       #run the main method

...
class ITLA:
    ...
    def __connect(self, port, baud):
        ...
        if (os.name != 'posix'):
            self.__link = serial.Serial('com' + str(port), baud, timeout = 0.06)
        else:
            self.__link = serial.Serial('/dev/ttyS' + str(port), baud, timeout = 0.06)
    ...
    def connect(self, port = 1, baud = None):
        ...
        if (self.__OIFlogging):
            ...
            oiflogfile = open(self.__commslogfilename, "a")
            oiflogfile.write("# connecting to port " + str(port) + " at " + time.asctime().replace(" ", "_").replace(':','-') + "\n")
        else:
    ...
    def read(self, byte_count = 1, maxRetry = 8, beginning=True, incomplete=False):
        string = ''
        retries = 0
    ...
    def logentry(self,string=None):
        if (self.__OIFlogging):
            oiflogfile = open(self.__commslogfilename, "a")
            oiflogfile.write("# " + string + "\n")
        else:
            print self.hilite("Warning: Not logging OIF transactions. Use\n it.logfile(<filename>) to set log filename and\n it.logging(True) to enable logging",False,True)
            print 'Unable to write log entry'
        return
    ...
    def __upgradeFirmware(self, filename, version):
        ...
        if curr_laser == 0:			# single uITLA
            ...
            time.sleep(1)		#change to 0.2 sec
            self.__link.flushInput()
            # self.flushBuffer()
    ...
    def dbgLocks(self):
        'Return tuple (status_string, register)'
        return self.register(Register.Register(Register.NOP_STATS, 1))

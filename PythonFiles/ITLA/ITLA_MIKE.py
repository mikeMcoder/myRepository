...
class ITLA:
    ...
    def readx99(self):
        self.write(''.join(map(chr,[0x40, 0x99, 0x12, 0x34])))
    ...
    def setpassword(self):
        self.write(''.join(map(chr,[0x11, 0x99, 0x00, 0x00])))
        level = self.read(4)
        self.write(''.join(map(chr,[0xA1, 0x99, 0x02, 0x09])))
        level = self.read(4)
        self.write(''.join(map(chr,[0x31, 0x99, 0xEE, 0x75])))
        level = self.read(4)
        self.write(''.join(map(chr,[0x71, 0x99, 0xC3, 0xB2])))
        level = self.read(4)
        print "Password level", ord(level[3])
    ...
    def write(self, buffer):
        ...
        if (self.__OIFlogging):
            oiflogfile = open(self.__commslogfilename, "a")
            exactTime = time.asctime()
            oiflogfile.write(exactTime + '\t')
    ...
    def __upgradeFirmware(self, filename, version):
        ...
        time.sleep(0.5)     # Allow laser 2 to boot --> drive laser 1
        self.laser(1)
        status, NA = self.dlConfig(init_run = 1, runv = type)
        if (status != 'OK'):

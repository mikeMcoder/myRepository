...
class ITLA:
    ...
    def disconnect(self):
        ...
        self.__baud = 0
        if self.__link != None:
            print 'Releasing the serial port: Need to connect again via it.connect'
            self.__link.close()
            self.__link = None
    ...
    def read(self, byte_count = 4, maxRetry = 1):
        string = ''
        retries = 0
        while(retries < maxRetry):
            ...
            if (len(string) >= byte_count):
                ...
                #print 'Returned %d bytes' %len(string)
                return string
            retries += 1
            time.sleep(0.1)
    ...
    def ioCap(self, baudrate = None, module_select_no_reset = False):
    ...
    def statusF(self, fvsf = 0, ffreq = 0, ftherm = 0, fpwr = 0, xel = 0, cel = 0, mrl = 0, crl = 0, fvsfl = 0, ffreql = 0, ftherml = 0, fpwrl = 0):
        ...
        write = fvsf or ffreq or ftherm or fpwr or xel or cel or mrl or crl or fvsfl or ffreql or ftherml or fpwrl

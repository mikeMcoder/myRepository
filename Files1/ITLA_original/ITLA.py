...
class ITLA:
    ...
    def __init__(self, t_object = None):
        ...
        self.__baud = 0
        self.__port = 0
        self.__timeout = RX_TIMEOUT_DEFAULT
        self.__print_once = 1
        self.__link = None
        self.__packet = None
    ...
    def readx99(self):
        ...
        dataformat = ord(self.read(1, maxRetry=3, incomplete=True))
    ...
    def __connect(self, port, baud, RXtimeout=RX_TIMEOUT_DEFAULT):
        self.disconnect()
        self.__baud = baud
        self.__port = port
        self.__timeout = RXtimeout
        if (self.__OIFlogging):
        ...
        try:
            self.__link = serial.Serial(self.__portprefix + str(port), baud, timeout = RXtimeout)
        except:
    ...
    def connect(self, port = 1, baud = 9600, timeout = RX_TIMEOUT_DEFAULT):
        if self.__inTMode:
            return
        '''Connect to a port, return tuple (baud_rate, status_string)
        When baud is not give, it will try to auto detect.'''
        self.disconnect()
        self.__t_object.disconnect()   # disconnect t serial port
        print 'port:', port, 'baud:', baud, 'timeout:', timeout
   
        if baud :
            self.__baud = baud
            try:
                self.__connect(port, baud, timeout)
            except:
        ...
        else:       # baud 0 searches
            bauds = [9600, 115200, 57600, 19200, 38400]
    ...
    def register(self, register = None, write = 0):
        ...
        if register != None:
            ...
            if (register.address() == Register.DLCONFIG):
                Max_RXretry = RX_TIMEOUT_UPGRADE / self.__timeout
            if (register.address() == Register.EAR):
                Max_RXretry = RX_TIMEOUT_UPGRADE_EAR / self.__timeout
    ...
    def read(self, byte_count = 1, maxRetry = 1, beginning=True, incomplete=False): # call with maxRetry=0 to do a dummy read (for log file completeness)
        if byte_count == 0xFFFF:
            return self.__link.read(byte_count)         # flushBuffer()
    ...
    def packet(self, packet = None, computeChecksum = 1, RXretries = 1, MS_baud = TOGGLE_MS_BAUD):
        ...
        if packet != None:
            ...
            if self.__retrying:  # prevent retry recursion
                return None
    ...
    def monitor(self, addr = 0x80A8):   # Monitor version 1.2.x or 2.1.x
        ...
        if status == 'OK':
            ...
            word = response.data()
            message += struct.pack('BB', (word & 0xFF00) >> 8, 0)
        else:
    ...
    #it.writeAEAfloatarray( ITLA.Register.Register(address=0x85,data=0x0002), floatlist = [0.5,0.6],write=True)

    #floatbytes = map(ord,struct.pack('>f',-1342.45))
    #struct.unpack('>f',struct.pack('>H',0x100 * floatbytes[0] + floatbytes[1] ) + struct.pack('>H', 0x100 * floatbytes[2] + floatbytes[3] ))[0]
    ...
    def __upgradeFirmware(self, filename, version):
        ...
        if curr_laser == 0:			# single uITLA
            ...
            for i in range(0, len(text), 2):
                ...
                packet.checksum(packet.computedChecksum())
                self.write(packet.buffer())
                response = self.read(4, maxRetry = RX_TIMEOUT_UPGRADE_EAR / self.__timeout)
                if len(response) != 4:

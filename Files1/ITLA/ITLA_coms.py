...
class ITLA:
    ...
    def __init__(self, t_object = None):
        ...
        connecttimestring = time.asctime()
        strconnecttimestring = connecttimestring.replace(' ','')
        strconnecttimestring = strconnecttimestring.replace(':','')
        self.__commslogfilename = "commslog_" + strconnecttimestring + ".txt"
        self.__connecttime = time.time()
        oiflogfile = open(self.__commslogfilename, 'a')
        oiflogfile.write("#logging started " + strconnecttimestring )
    ...
    def write(self, buffer):
        ...
        oiflogfile = open(self.__commslogfilename, "a")
        oiflogfile.write("%04.4fs \t" %(time.time() - self.__connecttime))
        buff = ''
        for c in buffer:
            buff += str(hex(ord(c)) + '\t')
        buff.rstrip('\t')
        oiflogfile.write(buff)
    ...
    def read(self, byte_count = 1, maxRetry = 1):
        string = ''
        retries = 0
        while(retries < maxRetry):
            ...
            if (len(string) >= byte_count):
                ...
                for c in string:
                    strng += str(hex(ord(c)) + '\t')
                strng.rstrip('\t')
                oiflogfile.write( '\t' + strng + '\t')
                oiflogfile.write('%4.4fms \n'%writetimems)
                return string
            retries += 1
            time.sleep(0.1)

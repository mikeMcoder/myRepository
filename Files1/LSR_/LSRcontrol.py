...
class LSRcontrol(object):
    ...
    def __init__(self, t=None, it=None):
        ...
        if distutils.version.LooseVersion(pd.__version__) < distutils.version.LooseVersion('0.22.0'):
            print self.hilite('Pandas module is out of date.  Please upgrade to 0.22.0 or greater.', False, True)
    ...
    def _generatecaltable(self, cppfilename=None, content=None,nanoDevice=False, progress=False):
        if cppfilename is not None:
            with open(cppfilename) as myfile:
                content = myfile.read()
        if nanoDevice == True:
            tables = ['CalibDataRecNano']
        else:        
            tables = ['CalibDataRecABC','CalibDataRecMaster']
        parsedtables = []
        
        for tablename in tables:
            if 'Master' in tablename:
                remote = True
            else:
                remote = False
        ...
        if len(dups) > 0:
            print self.hilite("Warning: Duplicated calibration variable names : " + ' '.join(dups.variable), False, True)

        misaligned = self._caltable[ self._caltable.type.isin(['uint32_t','float']) & self._caltable.offset % 4 != 0 ]
...
#                    structList.append([row['type'],index-start-1,index])
    ...
    def _flattendicts(self, df, typelengths, varsep='__', base_struct_name='calibrationDictionary', progress=False):
        """
...
#                     for line in textlines if re.match('.*?' + classname + '::.*?', line)]
        ...
        if nanoDevice == True:
            ...
            df = self._flattendicts(df, typelengths, progress=progress)
...
#                print 'Setting ' + getsetvalue.info.variable + ' as invalid'
    ...
    def loadblankkv(self):
        self._get_cal_map_ifneeded()
        for keys,value in self.cal.CalibDataRecNano.__dict__.items():
            keys = keys.split('__')
            tmp = self.kv.kvdict
            for key in keys[:-1]:
    ...
    def load_kv_from_module(self, calcontainer=None, limitlist=[], defaultstonone=False):
        self._get_cal_map_ifneeded()
...
#            order = getattr( getattr( getattr( getattr( getattr(self, 'cal'), calbase), fullkey), 'info'), 'offset' )
    ...
    def load_kv_from_file(self,filename):
...
#            self.t._connect(skip=True)
    ...
    def modeltargets(self, targetfreq=None):
        if targetfreq is None:
            ...
            packet.packed.payload.float = targetfreq
    ...
    def _unlockheader(self, headernum):
        ...
        if self._keys[headernum] is not None:
            ...
            return self.sendpacket(packet, inunlock=True)
...
#            with open(bridgefilename ,'wb') as handle:
    ...
    def sendcommand(self, cmd, retries=3, inunlock=False, timeout=None):
        """
            ...
            try:
                ...
                if not inunlock:
                    try:
                        ...
                        if str(e) == 'Cal locked': # cal is locked.  Try to unlock and retry command                            
                            self._unlockheader(0)
                            reply = self.sendcommand(self._lastsentcmd, inunlock=True)
                else:
                    reply = self.getreply(timeout=timeout)
                return reply 
    ...
    def sendpacket(self, packet, inunlock=False):
        """

        :param packet: 

        """
        return self.sendcommand(str(bytearray(packet.rawdata)), inunlock=inunlock)
